...
import {async_test3} from './async_test3_component.js';
import {gridOptions} from "./async_test3_grid.js";
...
function exist_component_data(screen_id) {
    ...
    if (!customElements.get(webcomponent_id) && document.querySelector(webcomponent_id) == null) {
        // 컴포넌트가 없으면 만들기
        customElements.define(webcomponent_id, async_test3); // 웹 컴포넌트를 정의합니다.
        const test3Element = new async_test3(); // 웹 컴포넌트의 인스턴스를 생성합니다.
        // customElements.define(webcomponent_id, test1);
        // // 커스텀 엘리먼트 인스턴스 생성 및 문서에 추가
        // const test1Element = new test1();
        document.body.appendChild(test3Element);
        return false;
    }
