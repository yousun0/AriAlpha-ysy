...
from .models import Vin10100Actionsearch, VINM0200_GetCommCdMstr


def get_js_file(request, test_number=None):
    # static에서 js 파일 경로
    js_file_name = "Webcomponentwithdynamicjs/js/component_test_main.js"
...
async def get_html(request, test_number=None):
    ...
    return render(request, "Webcomponentwithdynamicjs/test1.html", context)
...
def get_grid(request):
    grid_file_name = "Webcomponentwithdynamicjs/js/component_test_grid.js"
    return JsonResponse({"grid_file_name": grid_file_name})


def get_css(request):
    css_file_name = "RDMT_SEARCH/css/RDMT_SEARCH_EL.css"
    return JsonResponse({"css_file_name": css_file_name})
...
def get_data(request, test_number=None):
    ...
    data = (
        Vin10100Actionsearch.objects.filter(dan_cd=dancode, yyyy_mm=yyyymm)
        .order_by("bldg", "room")
        .values(
            "bldg",
            "room",
    ...
    #         if not batch:
    #             break
    #         rows.extend([dict(zip(column_names, row)) for row in batch] * 1000)
    #
    # data = {"data": rows}
